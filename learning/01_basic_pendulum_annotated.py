...
class Example:
    ...
    def __init__(self, viewer, args=None):
        ...
        # 首先创建一个旋转，使摆在 XZ 平面内运动（侧面视图）
        # quat_from_axis_angle: 从 (旋转轴, 旋转角度) 创建四元数
        rot = wp.quat_from_axis_angle(wp.vec3(0.0, 0.0, 1.0), -wp.pi * 0.5)

        # 关节0：连接世界(parent=-1)到 link_0
        # parent=-1 表示固定到世界坐标系
        # axis: 旋转轴（在关节坐标系中）
        # parent_xform: 关节在父体上的安装位置和朝向
        #   - p=wp.vec3(0,0,5): 在世界坐标 z=5 的位置
        #   - q=rot: 应用旋转使摆在侧面
        # child_xform: 关节在子体上的安装位置
        #   - p=wp.vec3(-hx,0,0): 在连杆的左端 (x=-1.0)
        ...
        # parent_xform: 在 link_0 的右端 (x=+hx)
        # child_xform: 在 link_1 的左端 (x=-hx)
        ...
        # 【第6部分】正运动学初始化
        # ===================================================================
        # eval_fk (Forward Kinematics): 从关节坐标计算刚体位姿
        # 非 MuJoCo 求解器（XPBD、Featherstone 等）需要显式调用
        # 因为它们需要初始的 body_q 和 body_qd 来开始仿真
        # MuJoCo 求解器内部已经处理了这一步
